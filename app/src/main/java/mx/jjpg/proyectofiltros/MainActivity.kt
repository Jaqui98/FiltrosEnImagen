    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cmbOpciones1.onItemSelectedListener=object:AdapterView.OnItemSelectedListener{
            ...
            override  fun onItemSelected(parent:AdapterView<*>, view: View?, position:Int, id:Long ){
                ...
                bmap = (imagenId.getDrawable() as BitmapDrawable).bitmap
                var cambio: Bitmap
                Toast.makeText(
                    applicationContext,
                    "Seleccionado: $pos",
        ...
        cmbOpciones2.onItemSelectedListener=object:AdapterView.OnItemSelectedListener{
            override fun onNothingSelected( parent: AdapterView<*> ) {
                Toast.makeText( applicationContext, "Sin selección Efectos Convolucion", Toast.LENGTH_SHORT).show()
            }
            override  fun onItemSelected(parent:AdapterView<*>, view: View?, position:Int, id:Long ){
                val pos=parent.getItemAtPosition( position )
                //Se convierte la imagen a Bitmap para poder trabajarla con los filtros
        ...
        btnGuardar.setOnClickListener() {
            ...
            bmap = (imagenId.drawable as BitmapDrawable).bitmap
            bmap.compress(Bitmap.CompressFormat.JPEG, 100, bytes )
            val photoFile: File? = try {
                createImageFile()
            } catch (e: IOException){
                Toast.makeText(applicationContext, "No se guardo T-T", Toast.LENGTH_SHORT).show()
                null
            }
            if(photoFile != null){
                MediaStore.Images.Media.insertImage(this.contentResolver, bmap, currentPhotoPath, "Proyecto")
                Toast.makeText(applicationContext, "Imagen guardada con exito :3", Toast.LENGTH_SHORT).show()
            }
